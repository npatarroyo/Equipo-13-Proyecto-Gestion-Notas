...
def listar_cursos():
    resultado=controlador.listar_cursos()
    return jsonify(resultado)
...
def activar_cuenta():
    ...
    if username == '' or codver == '':
        flash('Usuario o codigo de verificacion incorrecto')
    else:
        ...
        if resultado==False:
            ...
            if(resultado[0]['verificado']==1):
                ...
                if check_password_hash(resultado[0]['passwd'],passwd):
                    ...
                    print(listadouser)
                    return render_template('usuarios.html',datauser=listadouser)
                else:
...
def add_materia():
    ...
    return redirect(url_for('menu_cursos'))  
